...
var __async = (__this, __arguments, generator) => {
	return new Promise((resolve2, reject) => {
		...
		var step = (x) =>
			x.done ? resolve2(x.value) : Promise.resolve(x.value).then(fulfilled, rejected);
					...
					cb.setPlaceholder('Folder')
						...
						.onChange((new_folder) => {
							if (
								...
								log_error(
									new TemplaterError('This folder already has a template associated with it')
								);
								return;
							}
	...
	add_user_script_functions_setting() {
		...
		if (!this.plugin.settings.user_scripts_folder) {
			...
			const files = errorWrapperSync(
				() => get_tfiles_from_folder(this.app, this.plugin.settings.user_scripts_folder),
...
var FuzzySuggester = class extends import_obsidian7.FuzzySuggestModal {
	...
	getItems() {
		...
		const files = errorWrapperSync(
			() => get_tfiles_from_folder(this.app, this.plugin.settings.templates_folder),
	...
	generate_object(new_config) {
		return __async(this, null, function* () {
			...
			return __spreadValues(
				__spreadValues({}, this.static_object),
...
var InternalModuleFile = class extends InternalModule {
	...
	generate_create_new() {
		return (template, filename, open_new = false, folder) =>
			__async(this, null, function* () {
				...
				const new_file = yield this.plugin.templater.create_new_note_from_template(
					template,
					folder,
					filename,
					open_new
	...
	generate_include() {
		return (include_link) =>
			__async(this, null, function* () {
						...
						if (cache) {
							...
							if (result) {
								inc_file_content = inc_file_content.slice(
									result.start.offset,
									(_a = result.end) == null ? void 0 : _a.offset
								);
							}
						}
					}
				...
				try {
					const parsed_content = yield this.plugin.templater.parser.parse_commands(
						inc_file_content,
		...
		return (size, query) =>
			__async(this, null, function* () {
				const response = yield this.getRequest(
					`https://source.unsplash.com/random/${size != null ? size : ''}?${
						query != null ? query : ''
					}`
				);
...
var InternalModuleFrontmatter = class extends InternalModule {
	...
	create_dynamic_templates() {
		return __async(this, null, function* () {
			const cache = this.app.metadataCache.getFileCache(this.config.target_file);
			this.dynamic_functions = new Map(
				Object.entries((cache == null ? void 0 : cache.frontmatter) || {})
			);
		});
	}
};
...
var InternalModuleSystem = class extends InternalModule {
	...
	generate_prompt() {
		return (prompt_text, default_value, throw_on_cancel = false) =>
			__async(this, null, function* () {
	...
	generate_suggester() {
		return (text_items, items, throw_on_cancel = false, placeholder = '', limit) =>
			__async(this, null, function* () {
				const suggester = new SuggesterModal(this.app, text_items, items, placeholder, limit);
				const promise = new Promise((resolve2, reject) =>
					suggester.openAndGetValue(resolve2, reject)
				);
				try {
					return yield promise;
				} catch (error) {
...
var UserSystemFunctions = class {
	...
	generate_system_functions(config2) {
		return __async(this, null, function* () {
			...
			const internal_functions_object =
				yield this.plugin.templater.functions_generator.generate_object(
					config2,
			...
			for (const template_pair of this.plugin.settings.templates_pairs) {
				...
				if (import_obsidian12.Platform.isMobileApp) {
					...
					user_system_functions.set(template, (user_args) =>
						__async(this, null, function* () {
		...
		return __async(this, null, function* () {
			...
			switch (functions_mode) {
				...
				case 1:
					...
					Object.assign(
						final_object,
						__spreadProps(__spreadValues({}, internal_functions_object), {
...
function parse(str, config2) {
	...
	var parseCloseReg = new RegExp(
		'\'|"|`|\\/\\*|(\\s*(-|_)?' + escapeRegExp(config2.tags[1]) + ')',
		'g'
	);
	var m;
...
function compileToString(str, config2) {
	...
	var res =
		"var tR='',__l,__lP" +
		(config2.include ? ',include=E.include.bind(E)' : '') +
		(config2.includeFile ? ',includeFile=E.includeFile.bind(E)' : '') +
		'\nfunction layout(p,d){__l=p;__lP=d}\n' +
		(config2.globalAwait ? 'const _prs = [];\n' : '') +
		(config2.useWith ? 'with(' + config2.varName + '||{}){' : '') +
		compileScope(buffer, config2) +
		(config2.includeFile
			? 'if(__l)tR=' +
			  (config2.async ? 'await ' : '') +
			  ('includeFile(__l,Object.assign(' + config2.varName + ',{body:tR},__lP))\n')
			: config2.include
			? 'if(__l)tR=' +
			  (config2.async ? 'await ' : '') +
			  ('include(__l,Object.assign(' + config2.varName + ',{body:tR},__lP))\n')
			: '') +
...
function getWholeFilePath(name, parentfile, isDirectory) {
	var includePath =
		(0, import_path.resolve)(
			isDirectory ? parentfile : (0, import_path.dirname)(parentfile),
			name
		) + ((0, import_path.extname)(name) ? '' : '.eta');
	return includePath;
}
...
var Templater = class {
	...
	parse_template(config2, template_content) {
		return __async(this, null, function* () {
			const functions_object = yield this.functions_generator.generate_object(
				config2,
	...
	create_new_note_from_template(template, folder, filename, open_new_note = true) {
		return __async(this, null, function* () {
			...
			const created_note = yield this.app.fileManager.createNewMarkdownFile(
				folder,
			...
			if (template instanceof import_obsidian14.TFile) {
				...
				output_content = yield errorWrapper(
					() =>
						__async(this, null, function* () {
	...
	append_template_to_active_file(template_file) {
		return __async(this, null, function* () {
			...
			const output_content = yield errorWrapper(
				() =>
					__async(this, null, function* () {
						return this.read_and_parse_template(running_config);
					}),
			...
			while ((node = walker.nextNode())) {
				...
				if ((match = dynamic_command_regex.exec(content)) != null) {
					...
					if (!pass) {
						...
						functions_object = yield this.functions_generator.generate_object(
							config2,
							FunctionsMode.USER_INTERNAL
						);
						this.current_functions_object = functions_object;
					...
					while (match != null) {
						...
						const command_output = yield errorWrapper(
							() =>
								__async(this, null, function* () {
	...
	static on_file_creation(templater, file) {
		return __async(this, null, function* () {
			...
			const template_folder = (0, import_obsidian14.normalizePath)(
			...
			if (file.stat.size == 0 && templater.plugin.settings.enable_folder_templates) {
				...
				const template_file = yield errorWrapper(
					() =>
						__async(this, null, function* () {
...
var EventHandler = class {
	...
	update_trigger_file_on_creation() {
		if (this.settings.trigger_on_file_creation) {
			this.trigger_on_file_creation_event = this.app.vault.on('create', (file) =>
		...
		functions: {
			now: {
				...
				args: {
					...
					reference_format: { name: 'reference_format', description: 'The date reference format.' }
				}
			},
			tomorrow: {
				name: 'tomorrow',
	...
	file: {
		...
		functions: {
			...
			creation_date: {
				...
				args: {
					format: { name: 'format', description: 'Format for the date, refer to format reference' }
				}
			},
			cursor: {
				name: 'cursor',
			...
			cursor_append: {
				...
				args: {
					content: { name: 'content', description: 'The content to append after the active cursor' }
				}
			},
			exists: {
				name: 'exists',
			...
			rename: {
				...
				args: { new_title: { name: 'new_title', description: 'The new file title.' } }
			},
			selection: {
				name: 'selection',
	...
	system: {
		...
		functions: {
			...
			prompt: {
				...
				args: {
					prompt_text: { name: 'prompt_text', description: 'Text placed above the input field' },
					default_value: {
						name: 'default_value',
			...
			suggester: {
				...
				args: {
					...
					placeholder: { name: 'placeholder', description: 'Placeholder string of the prompt' },
					limit: {
						name: 'limit',
						description:
	...
	web: {
		...
		functions: {
			...
			random_picture: {
				...
				args: {
					size: { name: 'size', description: 'Image size in the format `<width>x<height>`' },
					query: {
						name: 'query',
						description:
	...
	registerCodeMirrorMode() {
		return __async(this, null, function* () {
			...
			if (overlay_mode == null) {
				log_error(
					new TemplaterError("Couldn't find customOverlayMode, can't enable syntax highlighting.")
				);
				return;
			}
